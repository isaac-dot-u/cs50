...
void add_pairs(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        for (int j = i + 1; j < candidate_count; j++)
        {
            ...
            // If preferences[i][j] == preferences[j][i], no pair is added (tie).
        }
    }
    return;
}
...
bool creates_cycle(int winner, int loser)
{
    // If loser leads back to winner, it's a cycle
    if (loser == winner)
    {
        return true;
    }

    // Check all candidates that loser points to
    for (int i = 0; i < candidate_count; i++)
    {
        if (locked[loser][i]) // loser → i edge exists
        {
...
void print_winner(void)
{
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        for (int j = 0; j < candidate_count; j++)
        {
            if (locked[j][i]) // if j → i is locked, then i has an incoming edge
            {
        ...
        // If nobody points to candidate i, they're the winner
